...
export function TirageCablesPage({ allData }: { allData: CableData[] }) {
  const baseData = useMemo(() => allData.filter(c => c.respTirage === 'GEST' && c.indApproCa === 'O'), [allData]);
  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in overflow-auto">
      {/* KPIs */}
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3 [&>div]:h-full">
